...
const formSchema = z.object({
  ...
  photo: z
    ...
    .refine((files) => files?.[0]?.size <= MAX_FILE_SIZE, "Maximum file size is 5 MB")
    .refine(
...
export default function TodoForm() {
  // useTransition replaces the manual isUploading state for Server Actions
  ...
  const onSubmit: SubmitHandler<IFormInput> = async (data) => {
    ...
    try {
      ...
      // 2. Upload to S3 using native fetch
  ...
  return (
    <form className="flex flex-col gap-3 w-full" onSubmit={handleSubmit(onSubmit)}>
      ...
      <div className="flex flex-col gap-1">
        ...
        <input type="text" id="firstName" className="border p-1.5 rounded" {...register("firstName")} />
        {errors.firstName && <span className="text-red-500 text-sm">{errors.firstName.message}</span>}
      </div>

      <div className="flex flex-col gap-1">
        <label htmlFor="lastName" className="text-sm">Last Name</label>
        <input type="text" id="lastName" className="border p-1.5 rounded" {...register("lastName")} />
        {errors.lastName && <span className="text-red-500 text-sm">{errors.lastName.message}</span>}
      </div>

      <div className="flex flex-col gap-1">
        <label htmlFor="photo" className="text-sm">Add Photo</label>
        <input type="file" id="photo" accept="image/jpeg,image/png,application/pdf" {...register("photo")} className="text-sm" />
